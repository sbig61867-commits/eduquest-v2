...
## Pending migrations (NOT yet applied — run manually before using the feature)

- `survey_migration.sql` — adds `surveys` / `survey_responses` tables + RLS for the in-platform pilot feedback survey (teacher creates per group, students answer once). Required before the "pilot" report scope or the student SurveyCard will show real data.
