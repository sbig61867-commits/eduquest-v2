...
export function StudentCoursesClient({ courses, progressMap, groupsByCourse = {}, pastCourses = [] }: Props) {
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-slate-400 mt-1">{t('centre')}</p>
        </div>
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-white">{t('title')}</h2>
        <p className="text-slate-400 mt-1">{t('enrolledSummary', { count: courses.length })}</p>
      </div>
            ...
            >
              ...
              <div className="flex items-start justify-between mb-3">
                ...
                <span className="text-xs px-2 py-1 rounded-full bg-slate-800 text-slate-400 flex items-center gap-1">
                  ...
                  {course.has_levels ? t('levelled') : t('flat')}
                </span>
              </div>
                ...
                <div className="mb-4 mt-auto">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs text-slate-400 flex items-center gap-1">
                      <TrendingUp className="w-3 h-3" /> {t('progress')}
                    </span>
                  ...
                  <p className="text-xs text-slate-500 mt-1">{t('itemsCompleted', { done: progress.completed, total: progress.total })}</p>
                </div>
              )}
              ...
              >
                {t('courseAction', { state: pct > 0 ? 'resume' : 'start' })}
              </Button>
            </div>
          )
        })}
      ...
      {pastCourses.length > 0 && (
        <div className="space-y-3">
          <h3 className="text-white font-semibold flex items-center gap-2">
            <History className="w-4 h-4 text-slate-400" /> {t('pastCourses')}
          </h3>
          <p className="text-slate-500 text-xs">
            {t('pastCoursesHint')}
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {pastCourses.map(past => {
              const total = past.frozen_total ?? 0
              const done = past.frozen_completed ?? 0
              const pct = total > 0 ? Math.round((done / total) * 100) : 0
              return (
                <div key={past.id} className="bg-slate-900/60 border border-slate-800 rounded-xl p-5 opacity-80">
                  <h4 className="text-slate-200 font-semibold">{past.from_course_title ?? t('courseFallback')}</h4>
                  <p className="text-slate-500 text-xs mt-0.5">{t('pastGroup', { name: past.from_group_name })}</p>
                  <div className="mt-3">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs text-slate-500">{t('frozen')}</span>
                      <span className="text-xs text-slate-400">{pct}%</span>
                    </div>
                    <div className="w-full bg-slate-800 rounded-full h-1.5">
                      <div className="bg-slate-500 h-1.5 rounded-full" style={{ width: `${pct}%` }} />
                    </div>
                    <p className="text-xs text-slate-500 mt-1">{t('itemsCompleted', { done, total })}</p>
                  </div>
                  <p className="text-slate-500 text-xs mt-3">
                    {past.to_course_title
                      ? t('movedTo', { date: formatDate(past.created_at, locale), course: past.to_course_title })
                      : t('movedOn', { date: formatDate(past.created_at, locale) })}
                  </p>
                </div>
              )
            })}
