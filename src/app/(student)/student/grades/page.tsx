  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400 mt-1">{submissions?.length ?? 0} graded exams</p>
      </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
            ...
            <p className="text-3xl font-bold text-emerald-400">
              {submissions?.filter(s => {
                const max = (s.exams as any)?.questions?.reduce((a: number, q: any) => a + q.points, 0) || 1
                return (s.score / max) * 100 >= 60
              }).length ?? 0}
            </p>
          </div>
        </div>
              ...
              {submissions.map(sub => {
                ...
                return (
                  <tr key={sub.id} className="hover:bg-slate-800/50 transition-colors">
                    <td className="px-5 py-4 text-white text-sm font-medium">{(sub.exams as any)?.title ?? '—'}</td>
