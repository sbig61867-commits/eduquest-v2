...
export function MessagesClient({ initialMessages }: { initialMessages: ContactMessage[] }) {
  const t = useTranslations('superAdmin.messages')
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400 mt-1">{t('summary', { total: messages.length, unread })}</p>
      </div>
      ...
      {messages.length === 0 ? (
        ...
        <div className="space-y-3">
          {messages.map(m => (
            <div key={m.id}
              className={`bg-slate-900 border rounded-xl p-5 ${m.is_read ? 'border-slate-800 opacity-75' : 'border-blue-500/30'}`}>
              <div className="flex items-start justify-between gap-3 mb-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="text-white font-semibold">{m.name}</p>
                    {!m.is_read && <span className="px-2 py-0.5 rounded-full bg-blue-600/20 text-blue-400 text-xs font-medium">{t('new')}</span>}
                  </div>
                  <button onClick={() => copyEmail(m.id, m.email)} aria-label={t('copyEmail')}
                ...
                <span className="text-slate-500 text-xs shrink-0">{formatDate(m.created_at, locale)}</span>
              </div>
