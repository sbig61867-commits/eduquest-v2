    ...
    <div className="space-y-6">
      ...
      {lessons.length === 0 ? (
        <div className="text-center py-20 bg-surface border border-border rounded-lg">
          ...
          <p className="text-fg-secondary mb-4">No lessons yet. Create your first lesson.</p>
          <Button onClick={openAdd} size="sm"><Sparkles className="w-4 h-4" /> Create with AI</Button>
        </div>
      ) : (
        <div className="space-y-3">
          {lessons.map(lesson => (
            <div key={lesson.id} className="bg-surface border border-border rounded-lg p-5 hover:border-border-strong transition-colors">
          ...
          <div className="bg-accent-subtle border border-blue-500/20 rounded-lg p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-accent text-sm font-medium">AI Lesson Generator</span>
              </div>
              <button
                type="button"
                onClick={() => setShowAiInstructions(p => !p)}
                className="text-xs text-fg-secondary hover:text-accent transition-colors underline underline-offset-2"
              >
            ...
            <div className="flex gap-2">
              <input value={aiTopic} onChange={e => setAiTopic(e.target.value)} placeholder="Enter topic (e.g. Photosynthesis, Present Tense...)" className="flex-1 px-3 py-2 rounded-lg bg-surface border border-border-strong text-fg placeholder-fg-muted text-sm focus:outline-none focus:ring-2 focus:ring-accent" />
              <select value={aiLevel} onChange={e => setAiLevel(e.target.value)} className="px-3 py-2 rounded-lg bg-surface border border-border-strong text-fg text-sm focus:outline-none focus:ring-2 focus:ring-accent">
            ...
            {showAiInstructions && (
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 rounded-lg bg-surface border border-border-strong text-fg placeholder-fg-muted text-sm focus:outline-none focus:ring-2 focus:ring-accent resize-none"
                />
                <p className="text-xs text-fg-muted text-right">{aiInstructions.length}/1000</p>
              </div>
            )}
          </div>
