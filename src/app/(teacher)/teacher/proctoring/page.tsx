...
const VIOLATION_ICONS: Record<string, any> = {
  tab_switch: Monitor,
...
export default async function ProctoringReportsPage() {
  ...
  const { data: submissions } = await supabase
    ...
    .select('*, exams!inner(title, teacher_id, proctoring_enabled), users(full_name, email)')
    .eq('exams.teacher_id', user?.id ?? '')
    .eq('exams.proctoring_enabled', true)
    .order('submitted_at', { ascending: false })

  const flagged = submissions?.filter(s => {
    const events = s.proctoring_events as any[] ?? []
    return events.length > 0
  }) ?? []

  const clean = submissions?.filter(s => {
    const events = s.proctoring_events as any[] ?? []
    return events.length === 0
  }) ?? []
    ...
    <div className="space-y-6">
      ...
      {!submissions?.length ? (
        ...
        <div className="space-y-4">
          {submissions.map(sub => {
            const events = sub.proctoring_events as any[] ?? []
                ...
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className="text-white font-semibold">{(sub.users as any)?.full_name ?? '—'}</h3>
                      <Badge variant={isFlagged ? 'red' : 'green'}>{isFlagged ? `${events.length} violations` : 'Clean'}</Badge>
                    </div>
                    <p className="text-slate-400 text-sm">{(sub.users as any)?.email} · {(sub.exams as any)?.title}</p>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {events.map((ev: any, i: number) => {
                        const Icon = VIOLATION_ICONS[ev.type] ?? AlertTriangle
                        return (
