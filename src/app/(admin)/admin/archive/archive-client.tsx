...
export function ArchiveClient({ rows }: { rows: ArchiveRow[] }) {
  ...
  async function restore(r: ArchiveRow) {
    if (!confirm(`استرجاع "${r.title}" من الأرشيف؟ سيعود ظاهراً في المنصة.`)) return
    setBusy(r.id)
    ...
    else alert((await res.json().catch(() => ({}))).error ?? 'فشل الاسترجاع')
    setBusy('')
  }
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <div>
        <h2 className="text-2xl font-bold text-white">أرشيف الجامعة</h2>
        <p className="text-slate-400 mt-1">
          السجل التاريخي الكامل — {items.length} فصلاً وكورساً ({archivedCount} مؤرشف). كل التسليمات والعلامات محفوظة للرجوع إليها.
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input value={q} onChange={e => setQ(e.target.value)} placeholder="بحث بالاسم أو المعلم..."
            className="w-full ps-3 pe-9 py-2 rounded-lg bg-slate-900 border border-slate-700 text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
        ...
        <div className="flex rounded-lg border border-slate-700 overflow-hidden">
          {([['all', 'الكل'], ['live', 'نشط'], ['archived', 'مؤرشف']] as const).map(([v, label]) => (
                ...
                <div className="flex items-start gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <Badge variant={r.kind === 'group' ? 'blue' : 'gray'}>{r.kind === 'group' ? 'فصل' : 'كورس'}</Badge>
                      {r.is_archived
                    ...
                    <p className="text-slate-500 text-xs mt-1 flex items-center gap-1.5 flex-wrap">
                      <GraduationCap className="w-3.5 h-3.5" /> {r.teacher_name ?? 'غير معروف'}
                    ...
                    <div className="flex items-center gap-3 mt-2 text-xs text-slate-400">
                      <span className="flex items-center gap-1"><Users className="w-3.5 h-3.5" />{r.student_count} طالب</span>
                      {r.kind === 'group' && <span className="flex items-center gap-1"><BookOpen className="w-3.5 h-3.5" />{r.lesson_count} درس</span>}
                      <span className="flex items-center gap-1"><ClipboardList className="w-3.5 h-3.5" />{r.exam_count} اختبار/واجب</span>
                      <span className="text-emerald-400">{r.submission_count} تسليم</span>
                    </div>
                  </div>
                </div>
                {r.is_archived && (
