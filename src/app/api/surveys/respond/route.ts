...
export async function POST(request: Request) {
  ...
  if (profile?.role !== 'student' || !profile.tenant_id) return NextResponse.json({ ...(await apiErr('forbidden')) }, { status: 403 })
  ...
  if (!survey_id || typeof ease_rating !== 'number' || ease_rating < 1 || ease_rating > 5
    ...
    return NextResponse.json({ ...(await apiErr('missingOrInvalidFields')) }, { status: 400 })
  }
  ...
  if (!survey || !survey.is_open) return NextResponse.json({ ...(await apiErr('surveyNotOpen')) }, { status: 404 })

  const { data: membership } = await admin
    .from('group_students').select('group_id').eq('group_id', survey.group_id).eq('student_id', user.id).maybeSingle()
  if (!membership) return NextResponse.json({ ...(await apiErr('forbidden')) }, { status: 403 })
  ...
  if (dbErr) {
    if (dbErr.code === '23505') return NextResponse.json({ ...(await apiErr('alreadySubmitted')) }, { status: 409 })
    console.error('[surveys/respond POST]', dbErr)
    return NextResponse.json({ ...(await apiErr('answerSendFailed')) }, { status: 500 })
  }
