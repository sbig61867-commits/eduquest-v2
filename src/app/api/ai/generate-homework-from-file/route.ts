...
export async function POST(request: Request) {
  ...
  if (!sourceText) return NextResponse.json({ ...(await apiErr('noSourceText')) }, { status: 400 })
  ...
  if (allowed.size === 0) return NextResponse.json({ ...(await apiErr('noQuestionTypes')) }, { status: 400 })
