...
export async function POST(request: Request) {
  ...
  if (!rawTopic) return NextResponse.json({ error: 'الموضوع مطلوب' }, { status: 400 })
  if (rawTopic.length > 200) return NextResponse.json({ error: 'الموضوع طويل جداً (بحد أقصى 200 حرف)' }, { status: 400 })
  if (rawInstructions.length > 1000) return NextResponse.json({ error: 'التعليمات طويلة جداً (بحد أقصى 1000 حرف)' }, { status: 400 })

  // Sanitize both fields against prompt injection (same ruleset as generate-exam)
  const topic = rawTopic.replace(/[<>{}[\]`\\'"]/g, '').trim()
  if (!topic) return NextResponse.json({ error: 'الموضوع يحتوي على رموز غير مسموحة' }, { status: 400 })
  ...
  try {
    ...
    if (msg.includes('No AI provider configured')) {
      return NextResponse.json({ error: 'لم يُهيَّأ أي مزوّد ذكاء اصطناعي' }, { status: 503 })
    }
