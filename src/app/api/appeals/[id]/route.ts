...
export async function PATCH(
  ...
  if (!response || response.length < 3) {
    return NextResponse.json({ ...(await apiErr('responseRequired')) }, { status: 400 })
  }
  ...
  if (!appeal || appeal.tenant_id !== profile.tenant_id) {
    return NextResponse.json({ ...(await apiErr('appealNotFound')) }, { status: 404 })
  }

  const isOwner = appeal.teacher_id === user.id
  const isAdmin = profile.role === 'university_admin' || profile.role === 'super_admin'
  if (!isOwner && !isAdmin) return NextResponse.json({ ...(await apiErr('forbidden')) }, { status: 403 })
  if (appeal.status !== 'pending') {
    return NextResponse.json({ ...(await apiErr('appealDecided')) }, { status: 409 })
  }
  ...
  if (error) {
    ...
    return NextResponse.json({ ...(await apiErr('decisionSaveFailed')) }, { status: 500 })
  }
