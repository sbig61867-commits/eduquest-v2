      ...
      <div className="max-w-6xl mx-auto px-2 sm:px-6 h-16 flex items-center justify-between gap-1">
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          <div ref={ref} className="relative">
            <button
              onClick={() => setOpen(o => !o)}
              className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-[#dde7ee] text-[#486984] hover:border-[#c8d8e4] hover:text-[#0b3658] text-sm font-semibold transition-colors"
            >
              {lang === 'ar' ? 'AR' : 'EN'}
              <ChevronDown className={`w-3.5 h-3.5 transition-transform ${open ? 'rotate-180' : ''}`} />
            </button>
            {open && (
              <div className="absolute top-full mt-1.5 end-0 bg-white border border-[#dde7ee] rounded-[12px] shadow-[0_8px_24px_rgba(11,54,88,0.10)] overflow-hidden min-w-[96px] z-50">
                {(['ar', 'en'] as Lang[]).map(l => (
                  <button
                    key={l}
                    onClick={() => { setLang(l); setOpen(false) }}
                    className={`w-full text-start px-4 py-2.5 text-sm font-medium transition-colors ${lang === l ? 'bg-[#e6f1fa] text-[#0b3658]' : 'text-[#486984] hover:bg-[#f5f9fd] hover:text-[#0b3658]'}`}
                  >
