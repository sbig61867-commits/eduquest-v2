...
export function AppealReviewList({ appeals }: { appeals: AppealRow[] }) {
  ...
  async function resolve(id: string, status: 'upheld' | 'rejected') {
    ...
    if (response.length < 3) {
      toast.error(t('appeals.responseRequired'))
      return
    }
    setBusyId(id)
    try {
      ...
      if (!res.ok) { toast.error(data.error ?? t('appeals.saveError')); return }
      toast.success(status === 'upheld' ? t('appeals.upheld') : t('appeals.rejected'))
      router.refresh()
    } catch {
      toast.error(t('appeals.saveError'))
    } finally {
      setBusyId(null)
    }
  }
  ...
  return (
    <div className="bg-slate-900 border border-amber-900/40 rounded-xl p-5 space-y-4">
      <div className="flex items-center gap-2">
        ...
        <p className="text-white font-semibold">{t('appeals.pendingTitle')}</p>
