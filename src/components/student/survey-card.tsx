...
export function SurveyCard() {
  ...
  return (
    <>
      <div className="bg-gradient-to-l from-blue-600/20 to-blue-600/5 border border-blue-500/30 rounded-xl p-5 flex items-center justify-between gap-4">
            ...
            <div className="flex gap-1">
              {[1, 2, 3, 4, 5].map(n => (
                <button key={n} type="button" onClick={() => setEaseRating(n)} className="p-1">
                  <Star className={`w-7 h-7 ${n <= easeRating ? 'fill-amber-400 text-amber-400' : 'text-slate-600'}`} />
                </button>
              ))}
            </div>
          </div>
